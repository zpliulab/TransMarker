...
#gene_list = pd.read_csv(path + '/0.RealData/Filtered_Cleaned_Inputs/Union_Genes_'+GN+'.csv', header=None, names=["Gene"])  # All gene names
gene_list = pd.read_csv(path + '/0.RealData/Filtered_Cleaned_Inputs/Union_Genes_'+GN+'.csv')  # All gene names
Biomarker_nodes = pd.read_csv(path + '/4.BiomarkerIdentification/best_component_nodes_Reg_' + GN + '.txt', header=None, names=["Gene"])
...
with open('./evaluation_results_Reg_ExpFea_'+GN+'.txt', "w") as f:
...
roc_data.to_csv('./roc_curve_data_Reg_ExpFea'+GN+'.csv', index=False)
...
plt.savefig('ROC_plot_ExpFea_'+GN+'.pdf')  # Save as PDF
# Show the plot
plt.show()
