...
gene_list = pd.read_csv(path + '/0.RealData/ESCC/cellIDs_by_stage/Filtered_Cleaned_Inputs/Union_Genes_'+GN+'.csv', header=None, names=["Gene"])  # All gene names
Biomarker_nodes = pd.read_csv(path + '/4.BiomarkerIdentification/ESCC_best_component_nodes_Reg_' + GN + '.txt', header=None, names=["Gene"])
...
stage_list = ['N', 'HGIN', 'INF', 'Stage_I', 'Stage_II', 'Stage_III']  # Define the ordered stages
expression_matrices = []
labels = []
...
for ST in stage_list:
    ...
    expr_stage1_path = os.path.join(path, f"{ST}_{GN}T_M_cleaned_Filtered_UnionGenes.csv")
...
y_train_cat = to_categorical(np.argmax(y_train, axis=1), num_classes=6)
y_val_cat = to_categorical(np.argmax(y_val, axis=1), num_classes=6)
y_test_cat = to_categorical(np.argmax(y_test, axis=1), num_classes=6)
...
model.add(Dense(6, activation='softmax'))  # 6 classes for stages
...
with open('./evaluation_results_ESCC_'+GN+'.txt', "w") as f:
    f.write(evaluation_results)

print("Evaluation results saved to 'evaluation_results_ESCC.txt'")
...
roc_data = pd.DataFrame({
    ...
    'Pred_Prob_Class_6': y_pred_probs[:, 5]
})

roc_data.to_csv('./roc_curve_data_ESCC_'+GN+'.csv', index=False)
...
for i in range(6):  # Since there are 5 classes (stages)
...
plt.savefig('ROC_plot_ESCC_'+GN+'.pdf')  # Save as PDF
# Show the plot
plt.show()
