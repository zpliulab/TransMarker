...
gene_list = pd.read_csv(path+"/0.RealData/Filtered_Cleaned_Inputs/Union_Genes_"+GN+".csv", header=0, names=["Gene"])  # Assuming gene names
GW_scores = np.loadtxt(path + '/3.GWAlignment/gw_cumulative_alignment_Reg_'+GN+'.csv', delimiter=",")
...
# Option 1: Top 1%
genes_top_k, scores_top_k = filter_top_k_percent(alignment_scores, gene_names, percent=15)
print(genes_top_k)
print(scores_top_k)
...
regnet_adj_path = os.path.join(path, "0.RealData", "Filtered_Cleaned_Inputs", f"RegNetwork_Adj_UnionGenes_{GN}.csv")
...
nx.write_edgelist(best_component, 'best_component_edges_Reg_'+GN+'.txt')

# Save the node list of the best component to a text file
with open('best_component_nodes_Reg_' + GN + '.txt', 'w') as f:
